...
// ValidApprovalMode reports whether a persisted session approval mode is known.
func ValidApprovalMode(mode string) bool {
	switch mode {
	case "manual", "auto", "full_access":
		return true
	default:
		return false
	}
...
// The application service fills defaults for zero values.
...
func validateCreateOptions(opts CreateOptions) error {
	...
	if opts.ApprovalMode != "" &&
		!ValidApprovalMode(opts.ApprovalMode) {
