...
func TestPlanAndProjectPreserveLatestTurn(t *testing.T) {
	events := []Event{
		messageEventForTest(1, RoleUser, "old question"),
		messageEventForTest(2, RoleAssistant, "old answer"),
		messageEventForTest(3, RoleUser, "current question"),
	}
...
func TestInvalidCheckpointFallsBackToRawHistory(t *testing.T) {
	events := []Event{
		messageEventForTest(1, RoleUser, "question"),
		messageEventForTest(2, RoleAssistant, "answer"),
	}
...
func TestToolResultPolicyKeepsNewestWorkingResult(t *testing.T) {
	messages := []Message{
		{Role: RoleTool, ToolCallID: "old", Content: strings.Repeat("old ", 2_000)},
		{Role: RoleTool, ToolCallID: "new", Content: strings.Repeat("new ", 2_000)},
	}
...
func TestRollingPlanMeasuresCurrentProjection(t *testing.T) {
	events := []Event{
		messageEventForTest(1, RoleUser, strings.Repeat("old question ", 500)),
		messageEventForTest(2, RoleAssistant, strings.Repeat("old answer ", 500)),
		messageEventForTest(3, RoleUser, "follow-up"),
		messageEventForTest(4, RoleAssistant, strings.Repeat("new evidence ", 50)),
		messageEventForTest(5, RoleUser, "current"),
	}
	...
	want := EstimateMessagesTokens([]Message{
		CheckpointMessage(checkpoint.Summary),
		{Role: RoleUser, Content: "follow-up"},
		{Role: RoleAssistant, Content: strings.Repeat("new evidence ", 50)},
	})
...
func TestMidTurnPlanPreservesUserAnchorAndToolPairs(t *testing.T) {
	events := []Event{
		messageEventForTest(1, RoleUser, "old question"),
		messageEventForTest(2, RoleAssistant, "old answer"),
		messageEventForTest(3, RoleUser, "current request"),
		messageValueEventForTest(4, Message{
			Role: RoleAssistant,
			ToolCalls: []ToolCall{{
				ID: "call-1", Name: "read",
			}},
		}),
		messageValueEventForTest(5, Message{
			Role: RoleTool, ToolCallID: "call-1", Content: "first result",
		}),
		messageValueEventForTest(6, Message{
			Role: RoleAssistant,
			ToolCalls: []ToolCall{{
				ID: "call-2", Name: "test",
			}},
		}),
		messageValueEventForTest(7, Message{
			Role: RoleTool, ToolCallID: "call-2", Content: "second result",
		}),
		messageEventForTest(8, RoleAssistant, "latest working note"),
	}
...
func TestMidTurnPlanRejectsIncompleteToolPair(t *testing.T) {
	events := []Event{
		messageEventForTest(1, RoleUser, "current request"),
		messageValueEventForTest(2, Message{
			Role: RoleAssistant,
			ToolCalls: []ToolCall{{
				ID: "call-1", Name: "read",
			}},
		}),
		messageEventForTest(3, RoleAssistant, "later note"),
	}
...
func TestAutoPlanPrefersFurtherMidTurnCoverage(t *testing.T) {
	events := []Event{
		...
		messageEventForTest(2, RoleAssistant, "old answer"),
		messageEventForTest(3, RoleUser, "current request"),
		messageEventForTest(4, RoleAssistant, "completed step one"),
		messageEventForTest(5, RoleAssistant, "completed step two"),
		messageEventForTest(6, RoleAssistant, "latest working note"),
	}
...
func messageEventForTest(seq Seq, role Role, content string) Event {
	return messageValueEventForTest(seq, Message{Role: role, Content: content})
}

func messageValueEventForTest(seq Seq, value Message) Event {
	return Event{
		Seq:     seq,
		Kind:    KindMessageEnd,
		Session: "session-1",
