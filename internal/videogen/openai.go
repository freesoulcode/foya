// Package videogen provides video-generation adapters used by creative canvases.
package videogen
...
func (adapter *OpenAI) Generate(ctx context.Context, request Request) (Result, error) {
	if strings.TrimSpace(request.Prompt) == "" {
		return Result{}, errors.New("video prompt is required")
	}
	if strings.TrimSpace(request.Model) == "" {
		return Result{}, errors.New("video model is required")
	}
	if adapter.protocol != ProtocolSeedance && adapter.protocol != ProtocolMiniMaxH3 {
		return Result{}, fmt.Errorf("unsupported video protocol %q", adapter.protocol)
	}
	...
	for {
		...
		case "completed", "succeeded", "success":
			...
			if job.ID == "" {
				return Result{}, errors.New("video provider returned no job id")
			}
			return Result{}, errors.New("video provider completed without a result URL")
		...
		default:
			return Result{}, fmt.Errorf("video provider returned unknown status %q", job.Status)
		}
...
func (adapter *OpenAI) create(ctx context.Context, endpoint string, request Request) (videoJob, error) {
...
func (adapter *OpenAI) retrieve(ctx context.Context, endpoint, id string) (videoJob, error) {
...
func (adapter *OpenAI) doJob(req *http.Request) (videoJob, error) {
...
func (adapter *OpenAI) download(ctx context.Context, location string, authenticate bool) (Result, error) {
	...
	defer response.Body.Close()
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return Result{}, providerError(response)
	}
	...
	if int64(len(data)) > maxGeneratedVideoBytes {
		return Result{}, errors.New("generated video exceeds size limit")
	}
	...
	if !strings.HasPrefix(mediaType, "video/") {
		return Result{}, fmt.Errorf("generated result is not a video: %s", mediaType)
	}
	return Result{Data: data, MediaType: mediaType}, nil
}

func (adapter *OpenAI) newRequest(ctx context.Context, method, location string, body io.Reader) (*http.Request, error) {
...
func (adapter *OpenAI) createURL() string {
...
func (adapter *OpenAI) taskURL(endpoint, id string) string {
	if adapter.protocol == ProtocolMiniMaxH3 {
...
func (adapter *OpenAI) versionedBaseURL(version string) string {
...
func (adapter *OpenAI) seedanceBaseURL() string {
...
func (adapter *OpenAI) resolveDownloadURL(location string) (string, bool, error) {
