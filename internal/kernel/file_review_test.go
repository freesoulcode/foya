...
func TestFileReviewCanKeepOrUndoPendingChanges(t *testing.T) {
	t.Run("keep", func(t *testing.T) {
		...
		assertFileReviewEvent(t, be, sessionID, conversation.FileReviewKept)
	})
	...
	t.Run("undo", func(t *testing.T) {
		...
		assertNoPendingFileReview(t, be, sessionID)
		assertFileReviewEvent(t, be, sessionID, conversation.FileReviewUndone)
	})
}
...
func assertNoPendingFileReview(t *testing.T, be *Service, sessionID string) {
	t.Helper()
	review, err := be.PendingFileReview(context.Background(), sessionID)
	if err != nil {
		t.Fatal(err)
...
func assertFileReviewEvent(
	...
	resolved := lastEvent(events, conversation.KindFileReviewResolved)
	if resolved == nil {
		t.Fatalf("missing file_review_resolved event: %#v", eventKinds(events))
	}
	payload, ok := resolved.Payload.(conversation.FileReviewResolved)
