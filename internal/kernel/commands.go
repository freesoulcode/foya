...
func (b *Service) commandManager() (*workflow.CommandManager, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
	if b.commands == nil {
...
func (b *Service) commandProject(projectID string) (string, error) {
	if projectID == "" {
		return "", nil
	}
...
func (b *Service) workflowManager() (*workflow.Manager, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
	if b.workflows == nil {
...
func (b *Service) Commands(
	...
	switch workflow.Scope(scope) {
	case workflow.ScopeGlobal:
		return manager.ListScope(ctx, workflow.ScopeGlobal, "", "")
	case workflow.ScopeProject:
		return manager.ListScope(ctx, workflow.ScopeProject, projectID, projectPath)
	default:
		if scope == "effective" {
			return manager.List(ctx, projectID, projectPath)
		}
		return nil, workflow.ErrInvalidScope
	}
}

func (b *Service) SessionCommands(ctx context.Context, sessionID string) ([]workflow.Command, error) {
	...
	out := make([]workflow.Command, 0, len(items))
...
func (b *Service) UpdateCommand(
	ctx context.Context,
	ref string,
	input workflow.UpdateInput,
) (workflow.Command, error) {
	manager, err := b.commandManager()
	if err != nil {
		return workflow.Command{}, err
...
func (b *Service) ExecuteCommand(
	...
	case workflow.KindWorkflow:
		...
		if record.Kind == workflow.KindPlan {
			if _, err := b.sessions.SetAgentMode(sessionID, conversation.AgentModePlan, conversation.AgentModeExecute); err != nil {
				return CommandExecution{}, err
			}
...
func (b *Service) CompleteWorkflow(sessionID, content string) error {
	manager, err := b.workflowManager()
	if err != nil {
		return err
	}
	...
	if record.Kind == workflow.KindPlan {
		if _, err := b.sessions.SetAgentMode(sessionID, conversation.AgentModePlanReady, conversation.AgentModeExecute); err != nil {
			return err
		}
...
func (b *Service) Workflow(sessionID string) (workflow.Record, bool, error) {
	if _, ok := b.sessions.Get(sessionID); !ok {
		return workflow.Record{}, false, conversation.ErrNotFound
	}
	manager, err := b.workflowManager()
	if err != nil {
...
func (b *Service) CloseWorkflow(
	ctx context.Context,
	sessionID, id string,
) (workflow.Record, error) {
	current, ok := b.sessions.Get(sessionID)
	if !ok {
		return workflow.Record{}, conversation.ErrNotFound
	}
	manager, err := b.workflowManager()
	if err != nil {
	...
	if record.Kind == workflow.KindPlan {
		...
		if mode == "" || mode == conversation.AgentModePlan || mode == conversation.AgentModePlanReady {
			mode = conversation.AgentModeExecute
		}
...
func (b *Service) ApproveWorkflow(ctx context.Context, sessionID, id string) (WorkflowApproval, error) {
	manager, err := b.workflowManager()
	if err != nil {
	...
	var specTasks []conversation.Task
	...
	if record.Kind == workflow.KindPlan {
		if _, err := b.sessions.SetAgentMode(sessionID, conversation.AgentModeExecute, ""); err != nil {
			return WorkflowApproval{}, err
		}
	}
...
func (b *Service) broadcastWorkflow(ctx context.Context, record workflow.Record) {
	ev := conversation.Event{Kind: conversation.KindWorkflowUpdated, Session: record.SessionID, Time: time.Now(), Payload: record}
