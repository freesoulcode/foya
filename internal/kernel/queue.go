...
type Submission struct {
	Status string
	Queued *conversation.QueuedMessage
}
...
type turnScheduler struct {
	mu         sync.Mutex
	queues     map[string][]conversation.QueuedMessage
...
func newTurnScheduler() *turnScheduler {
	return &turnScheduler{
		queues:     make(map[string][]conversation.QueuedMessage),
...
func (b *Service) SubmitTurn(ctx context.Context, sessionID, text string) (Submission, error) {
	return b.SubmitInput(ctx, sessionID, conversation.UserInput{Text: text})
}

// SubmitChatInput exposes message submission to platform-neutral chat adapters.
func (b *Service) SubmitChatInput(
	ctx context.Context,
	sessionID string,
...
func (b *Service) SubmitInput(
	...
	if _, running := b.turns.runners[sessionID]; running || b.turns.compacting[sessionID] {
		item := conversation.NewQueuedMessage(sessionID, input, len(b.turns.queues[sessionID]))
...
func normalizeBrowserElements(
	elements []conversation.BrowserElement,
) ([]conversation.BrowserElement, error) {
	if len(elements) > maxBrowserElements {
		return nil, fmt.Errorf("at most %d browser elements are allowed", maxBrowserElements)
	}
	out := make([]conversation.BrowserElement, 0, len(elements))
	total := 0
...
func (b *Service) EnqueueMessage(ctx context.Context, sessionID, text string) (conversation.QueuedMessage, error) {
	return b.EnqueueInput(ctx, sessionID, conversation.UserInput{Text: text})
}

func (b *Service) EnqueueInput(
	ctx context.Context,
	sessionID string,
	input conversation.UserInput,
...
func (b *Service) validateImageInput(
	sessionID string,
	refs []conversation.AttachmentRef,
) error {
...
func (b *Service) ListQueuedMessages(sessionID string) ([]conversation.QueuedMessage, error) {
	if _, ok := b.sessions.Get(sessionID); !ok {
...
func (b *Service) UpdateQueuedMessage(
	ctx context.Context,
	...
	if index < 0 {
		return conversation.QueuedMessage{}, ErrQueuedMessageNotFound
	}
	if position != nil && (*position < 0 || *position >= len(items)) {
		return conversation.QueuedMessage{}, ErrInvalidQueuePosition
	}
	if text != nil {
	...
	if position != nil && *position != index {
		...
		target := *position
		items = append(items, conversation.QueuedMessage{})
...
func (b *Service) DeleteQueuedMessage(ctx context.Context, sessionID, messageID string) error {
	if _, ok := b.sessions.Get(sessionID); !ok {
		return conversation.ErrNotFound
	}
...
func (b *Service) DispatchQueuedMessage(
	ctx context.Context,
	sessionID, messageID string,
) (conversation.QueuedMessage, error) {
	if _, ok := b.sessions.Get(sessionID); !ok {
	...
	if runner := b.turns.runners[sessionID]; runner != nil {
		if index > 0 {
			items = append(items[:index], items[index+1:]...)
			items = append([]conversation.QueuedMessage{selected}, items...)
			normalizePositions(items)
			b.turns.queues[sessionID] = items
		}
...
func (b *Service) cancelCurrentTurn(sessionID string) {
	b.turns.mu.Lock()
...
func (b *Service) RewindTurn(
	ctx context.Context,
	sessionID string,
	targetUserSeq conversation.Seq,
	confirm bool,
	expectedHeadSeq conversation.Seq,
	expectedFileState string,
	forceFileKeys []string,
) (RewindSubmission, error) {
	if _, ok := b.sessions.Get(sessionID); !ok {
		return RewindSubmission{}, conversation.ErrNotFound
	}
	...
	if err != nil {
		b.turns.mu.Unlock()
		switch {
		case errors.Is(err, conversation.ErrActiveUserMessageNotFound):
			return RewindSubmission{}, ErrActiveUserMessageNotFound
		default:
			return RewindSubmission{}, err
		}
	...
	if err != nil {
		...
		case errors.Is(err, conversation.ErrActiveUserMessageNotFound):
			return RewindSubmission{}, ErrActiveUserMessageNotFound
		case errors.Is(err, conversation.ErrHistoryChanged):
			return RewindSubmission{}, ErrHistoryChanged
		default:
			return RewindSubmission{}, err
		}
...
func (b *Service) stopSessionAndWait(sessionID string, timeout time.Duration) {
	b.turns.mu.Lock()
...
func (b *Service) createRunnerLocked(sessionID string) (*sessionRunner, context.Context) {
...
func queueInput(item conversation.QueuedMessage) conversation.UserInput {
...
func (b *Service) popQueueLocked(sessionID string) conversation.QueuedMessage {
	items := b.turns.queues[sessionID]
	next := items[0]
	items = items[1:]
	normalizePositions(items)
...
func (b *Service) setQueueLocked(sessionID string, items []conversation.QueuedMessage) {
	if len(items) == 0 {
		delete(b.turns.queues, sessionID)
		return
	}
...
func (b *Service) broadcastQueueLocked(sessionID string) {
	snapshot := conversation.QueueSnapshot{Items: cloneQueue(b.turns.queues[sessionID])}
	ev := conversation.Event{
		Kind:    conversation.KindQueueUpdated,
		Session: sessionID,
		Time:    time.Now(),
		Payload: snapshot,
	}
...
func queueIndex(items []conversation.QueuedMessage, id string) int {
	for i := range items {
		if items[i].ID == id {
			return i
		}
...
func normalizePositions(items []conversation.QueuedMessage) {
	for i := range items {
		items[i].Position = i
	}
}
