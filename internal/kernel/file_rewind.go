...
func (b *Service) displayRewindFiles(
	sessionID string,
...
func inspectFileRewind(
	ctx context.Context,
	blobs conversation.Store,
	changes []conversation.RewindFileChange,
) ([]fileRewindCandidate, string, error) {
	grouped := make(map[string][]conversation.RewindFileChange)
	order := make([]string, 0)
...
func applyFileRewind(
	candidates []fileRewindCandidate,
	forceKeys []string,
) ([]conversation.RewindFileResult, func() error, error) {
...
func selectFileRewindPlans(
	candidates []fileRewindCandidate,
	forceKeys []string,
) ([]conversation.RewindFileResult, []fileRewindPlan, error) {
	force := make(map[string]struct{}, len(forceKeys))
	for _, key := range forceKeys {
		force[key] = struct{}{}
	}
	plans := make([]fileRewindPlan, 0, len(candidates))
	results := make([]conversation.RewindFileResult, 0, len(candidates))
	for _, candidate := range candidates {
		...
		case RewindFileReady:
			...
			results = append(results, conversation.RewindFileResult{
				Path:   candidate.path,
				Action: conversation.RewindFileRestored,
			})
		case RewindFileMergeable:
			plans = append(plans, candidate.safePlan)
			results = append(results, conversation.RewindFileResult{
				Path:   candidate.path,
				Action: conversation.RewindFileMerged,
			})
		case RewindFileModified:
			if _, selected := force[candidate.key]; selected {
				...
				results = append(results, conversation.RewindFileResult{
					Path:   candidate.path,
					Action: conversation.RewindFileForceRestored,
				})
			} else {
				results = append(results, conversation.RewindFileResult{
					Path:   candidate.path,
					Action: conversation.RewindFileKept,
				})
			}
		default:
...
func fileRewindBackups(plans []fileRewindPlan) []conversation.FileRewindBackup {
	backups := make([]conversation.FileRewindBackup, 0, len(plans))
	for _, plan := range plans {
		backup := conversation.FileRewindBackup{
			Path:          plan.path,
...
func RecoverFileRewinds(ctx context.Context, store conversation.Store) error {
	journals, err := store.PendingFileRewinds(ctx)
	if err != nil {
		return err
	}
	for _, journal := range journals {
		if journal.State == conversation.FileRewindPrepared {
		...
		} else if journal.State != conversation.FileRewindCommitted {
