// Package interaction coordinates user decisions that suspend agent execution.
//
// Approval requests and structured questions are exposed as asynchronous
// event/request pairs and may be resolved by any connected client.
package interaction
...
func NewGateway(bus *broker.Broker[conversation.Event], log conversation.Log) Gateway {
	return &gateway{
...
func (g *gateway) Request(ctx context.Context, req Request) (decision Decision, resultErr error) {
	...
	ev := conversation.Event{
		Kind:    conversation.KindApprovalReq,
		Session: req.Session,
		Time:    time.Now(),
		Payload: req,
	}
	...
	select {
	case d := <-ch:
		resolvedEv := conversation.Event{
			Kind:    conversation.KindApprovalResolved,
			Session: req.Session,
			Time:    time.Now(),
			Payload: struct {
				ID       string   `json:"id"`
