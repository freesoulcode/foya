...
func (ContextCompiler) Compile(input CompileInput) CompiledContext {
	messages := append([]message.Message(nil), input.Messages...)
	tools := append([]provider.ToolDef(nil), input.Tools...)
	var contextState *provider.ContextState
...
func estimateLayers(
	...
	var pinned, history, working []message.Message
	for i, item := range messages {
		switch {
		case i == 0 && item.Role == message.RoleSystem:
			pinned = append(pinned, item)
		case lastUser >= 0 && i >= lastUser:
			working = append(working, item)
		case item.Role == message.RoleSystem:
...
func estimateMessageLayer(messages []message.Message) int64 {
	if len(messages) == 0 {
		return 0
	}
...
func estimateToolLayer(tools []provider.ToolDef) int64 {
	if len(tools) == 0 {
		return 0
	}
...
func estimateNativeStateLayer(state *provider.ContextState) int64 {
	if state == nil {
		return 0
	}
