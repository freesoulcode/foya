...
func persistEventFileChangeTx(
	...
	if !filepath.IsAbs(change.Path) || change.AfterBlob == "" {
		return event.Event{}, errors.New("file change metadata is invalid")
	}
	if change.BeforeExists && change.BeforeBlob == "" {
		return event.Event{}, errors.New("file change before blob is missing")
	}
	...
	if change.ContentCaptured {
		...
		if !available {
			if ev.Kind != event.KindMessageImported {
				return event.Event{}, errors.New("file change references missing blobs")
			}
			item.FileChange = nil
			ev.Payload = item
			return ev, nil
...
func pruneFileChangesTx(
	...
	`, session, string(message.RoleUser), fileCheckpointLimit).Scan(&floor); err != nil {
