...
func TestGenerateCreatesPollsAndDownloadsVideo(t *testing.T) {
	...
	adapter := NewOpenAIVideoGenerator(VideoGeneratorConfig{BaseURL: server.URL + "/v1", APIKey: "secret", Protocol: ProtocolSeedance})
	adapter.pollInterval = time.Millisecond
	result, err := adapter.Generate(context.Background(), VideoGenerationRequest{
...
func TestGenerateReturnsProviderFailure(t *testing.T) {
	...
	adapter := NewOpenAIVideoGenerator(VideoGeneratorConfig{BaseURL: server.URL, Protocol: ProtocolSeedance})
	adapter.pollInterval = time.Millisecond
	_, err := adapter.Generate(context.Background(), VideoGenerationRequest{Model: "video-model", Prompt: "prompt"})
