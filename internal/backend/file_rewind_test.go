...
func TestApplyFileRewindRestoresSelectedFilesAndCanCompensate(t *testing.T) {
	...
	if len(results) != 2 ||
		results[0].Action != event.RewindFileRestored ||
		results[1].Action != event.RewindFileKept {
...
func TestApplyFileRewindForceRestoresModifiedFile(t *testing.T) {
	...
	if len(results) != 1 || results[0].Action != event.RewindFileForceRestored {
		t.Fatalf("results = %#v", results)
	}
...
func TestRecoverCommittedFileRewindOnlyCleansJournal(t *testing.T) {
	...
	target, err := store.Append(ctx, event.Event{
		Kind:    event.KindMessageEnd,
		Session: "session-1",
		Time:    time.Now(),
		Payload: message.Message{Role: message.RoleUser, Content: "request"},
	})
	if err != nil {
		t.Fatal(err)
	}
