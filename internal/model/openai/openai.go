...
type Provider struct {
	...
	client        oai.Client
	mu            sync.RWMutex
	capabilities  map[string]modelapi.ModelCapabilities
}
...
func New(cfg Config) *Provider {
	...
	return &Provider{
		...
		capabilities:  make(map[string]modelapi.ModelCapabilities),
	}
}
...
func (p *Provider) ModelCapabilities(model string) modelapi.ModelCapabilities {
	p.mu.RLock()
	...
	if !p.official {
		return modelapi.ModelCapabilities{}
	}
	...
	return modelapi.ModelCapabilities{ImageInput: boolPointer(true)}
}
...
func (p *Provider) SearchWeb(ctx context.Context, model, query string, limit int) ([]modelapi.SearchResult, error) {
	if model == "" {
		model = p.model
	}
	if model == "" || strings.TrimSpace(query) == "" {
		return nil, modelapi.ErrNativeSearchUnsupported
	}
	...
	if err != nil {
		return nil, fmt.Errorf("%w: %v", modelapi.ErrNativeSearchUnsupported, err)
	}
	if limit <= 0 {
		limit = 5
	}
	...
	results := make([]modelapi.SearchResult, 0, limit)
	for _, item := range response.Output {
		for _, content := range item.Content {
			for _, annotation := range content.Annotations {
				...
				results = append(results, modelapi.SearchResult{
...
func toChatMsgs(msgs []modelapi.InputMessage) []oai.ChatCompletionMessageParamUnion {
...
func inputText(parts []modelapi.InputPart) string {
	var values []string
...
func hasImagePart(parts []modelapi.InputPart) bool {
	for _, part := range parts {
...
func imageContentParts(parts []modelapi.InputPart) []oai.ChatCompletionContentPartUnionParam {
...
func userContentParts(parts []modelapi.InputPart) []oai.ChatCompletionContentPartUnionParam {
	...
	for _, part := range parts {
		...
		case "image":
			out = append(out, imageContentParts([]modelapi.InputPart{part})...)
		}
	}
	return out
}
...
func toChatToolDefs(tools []modelapi.ToolDef) []oai.ChatCompletionToolParam {
	if len(tools) == 0 {
		return nil
	}
...
func (p *Provider) Stream(ctx context.Context, req modelapi.Request) (<-chan modelapi.StreamEvent, error) {
	model := req.Model
	if model == "" {
		model = p.model
	}
	...
	go func() {
		...
		emit := func(ev modelapi.StreamEvent) bool {
			select {
			case <-ctx.Done():
				return false
			case ch <- ev:
		...
		for stream.Next() {
			...
			if chunk.JSON.Usage.Valid() && chunk.Usage.TotalTokens > 0 {
				...
				if !emit(modelapi.StreamEvent{Type: "usage", Usage: &usage}) {
					return
				}
			}
			for _, c := range chunk.Choices {
				...
				if reasoning := extractReasoning(c.Delta); reasoning != "" {
					if !emit(modelapi.StreamEvent{Type: "reasoning_delta", Text: reasoning}) {
						return
					}
				}
				// Text delta.
				if c.Delta.Content != "" {
					if !emit(modelapi.StreamEvent{Type: "text_delta", Text: c.Delta.Content}) {
						return
					}
				}
				// Tool-call delta assembled by index in the engine.
				for _, tc := range c.Delta.ToolCalls {
					if !emit(modelapi.StreamEvent{
		...
		if err := stream.Err(); err != nil {
			...
			ch <- modelapi.StreamEvent{Type: "error", Text: err.Error()}
			return
		}
		ch <- modelapi.StreamEvent{Type: "done", FinishReason: finishReason}
	}()
...
func (p *Provider) Complete(ctx context.Context, req modelapi.Request) (string, error) {
...
func (p *Provider) CompleteDetailed(
	...
	if model == "" {
		return modelapi.Completion{}, fmt.Errorf(
...
func (p *Provider) ListModels(ctx context.Context) ([]modelapi.ModelInfo, error) {
	page, err := p.client.Models.List(ctx)
	if err != nil {
		return nil, err
	}
	models := make([]modelapi.ModelInfo, 0, len(page.Data))
	capabilitiesByModel := make(map[string]modelapi.ModelCapabilities, len(page.Data))
	for _, m := range page.Data {
		if m.ID != "" {
			...
			models = append(models, modelapi.ModelInfo{
				ID:            m.ID,
...
var (
	_ modelapi.Provider           = (*Provider)(nil)
	_ modelapi.ModelLister        = (*Provider)(nil)
	_ modelapi.CapabilityResolver = (*Provider)(nil)
	_ modelapi.Completer          = (*Provider)(nil)
)
